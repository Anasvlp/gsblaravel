...
// Concatenate the letter and number and return the result
$id = $letter . $number;
return $id;
}
...
public function modifierVisiteur($id, $nom, $prenom, $adresse, $cp, $ville, $dateEmbauche)
{
	...
	public function getInfosVisiteurs()
	{
		...
		$res =  $this->monPdo->query($req);
		$les_visiteurs = $res ->fetchAll();
        return $les_visiteurs;
	}
	...
	public function getUnVisiteur($idVisiteur)
	{
		...
		$res =  $this->monPdo->query($req);
		$leVisiteur = $res ->fetch();
        return $leVisiteur;
	}  

	function MdpGeneration($nbChar) 
	{
	...
	function LoginGeneration($nom, $prenom) 
	{
		$login = '';
	...
	public function createPDF($les_visiteurs) {
		$data = $les_visiteurs::all();
		// share data to view
